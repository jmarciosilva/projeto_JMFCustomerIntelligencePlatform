<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-green-500">
            ...
            <p class="text-xs text-gray-500 mt-2">Comissões ganhas</p>
        </div>

        <!-- Conversions -->
        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-blue-500">
            <p class="text-gray-600 text-sm font-medium">Conversões</p>
            <p class="text-3xl font-bold mt-2">{{ $metrics['total_conversions'] ?? 0 }}</p>
            <p class="text-xs text-gray-500 mt-2">Vendas registradas</p>
        </div>

        <!-- Clicks -->
        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-purple-500">
            <p class="text-gray-600 text-sm font-medium">Clicks Totais</p>
            <p class="text-3xl font-bold mt-2">{{ number_format($metrics['total_clicks'] ?? 0, 0, ',', '.') }}</p>
            <p class="text-xs text-gray-500 mt-2">Links clicados</p>
        </div>

        <!-- CTR -->
        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-orange-500">
            <p class="text-gray-600 text-sm font-medium">CTR</p>
            <p class="text-3xl font-bold mt-2">{{ $metrics['ctr'] ?? 0 }}%</p>
            <p class="text-xs text-gray-500 mt-2">Taxa de cliques</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-white rounded-lg shadow p-6">
            <p class="text-gray-600 text-sm font-medium">EPC</p>
            <p class="text-2xl font-bold mt-2">R$ {{ number_format($metrics['epc'] ?? 0, 2, ',', '.') }}</p>
            <p class="text-xs text-gray-500 mt-2">Ganho por clique</p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <p class="text-xs text-gray-500 mt-2">Por conversão</p>
        </div>
